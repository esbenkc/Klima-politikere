...
screen_name_1000 <- df %>%
  ...
  unique %>%
  filter(tweet_count > 1000)

df <- df %>% 
  filter(screen_name %in% screen_name_1000$screen_name)
...
top_contents[top_contents$topic == "Topic 3",]$text
...
party_topic <- party_topic[party_topic$topic == 3,]
...
member_topic <- name_topic[name_topic$topic == "Topic 3",] %>%
...
name_topic[name_topic$name == "Dan Jørgensen" & name_topic$topic == 3,]$mean * 100
mean(name_topic[name_topic$topic == 3,]$mean) * 100
...
member_final <- inner_join(member_topic, word_freq_all) %>% 
  ...
  mutate(score = (score1 + score2 + score3 * 0.5)/2.5) %>% 
  arrange(desc(score))
