...
```{r}
df <- read_csv("mf_all_tweets_20200402.csv")
...
df %>%
  ...
  distinct() %>%
  ungroup() %>%
  mutate(tweet_count = color_bar("lightpink")(tweet_count),  
         screen_name = color_tile("lightblue","lightblue")(screen_name)) %>%
  my_kable_styling("Politikeres tweets")
```
...
removeSpecialChars <- function(x) gsub("[^a-zA-Z0-9#æøåÆØÅ]", " ", x)
...
```{r Party and topics}
df <- inner_join(df, polit, by = "screen_name")

# colnames(df)[7] <- "document"
# df$document <- as.character(df$document)
...
## Make circular diagram by member and topic

```{r fig.height=9, fig.width=9}
circos.clear()

plot_circle_topic <- chordDiagram(member_topic, transparency = .2)
...
top_contents[top_contents$topic == "Topic 11",]$text

```

## Model influence on topic 11
...
party_topic <- party_topic[party_topic$topic == 11,]
...
member_topic <- name_topic[name_topic$topic == 11,] %>%
...
name_topic[name_topic$name == "Dan Jørgensen" & name_topic$topic == 11,]$mean * 100
mean(name_topic[name_topic$topic == 11,]$mean) * 100
...
word_freq_all <- word_freq_all %>%
  mutate(data = sapply(data, function(x) x / sum(word_freq)))
...
```{r}
df <- read_csv("mf_all_tweets_20200402_sentiment.csv")
...
df <- df %>%
  select(screen_name, tweet_id, sentiment, text) %>%
  inner_join(polit[, c("name", "screen_name", "party")], by = "screen_name")

df$climate <- sapply(df$text, function(x) any(sapply(keywords, grepl, x)))

df <- df %>% inner_join(polit)
...
```{r}
party_final <- party_final %>% summarise(score = mean(score))
ggplot(party_final) +
...
member_final[member_final$party == "LG",]
```
